...
> **OTP ยืนยันอีเมล:** หน้าแรกต้องกรอกอีเมลและยืนยันด้วยรหัส OTP (ส่งทางอีเมลผ่าน Resend) ก่อนจึงเข้าฟอร์มได้ อีเมลที่ยืนยันจะถูกล็อกในฟอร์ม ถ้ายังไม่ตั้งค่า `RESEND_API_KEY` ระบบจะทำงานเป็น **โหมดทดสอบ** (แสดงรหัส OTP บนหน้าจอ) เมื่อตั้งค่าแล้วจะส่งเข้าอีเมลจริงและไม่โชว์รหัส
> ตัวแปร `TWILIO_*` เป็นทางเลือกสำหรับส่ง SMS (ระบบ OTP ปัจจุบันใช้อีเมล ไม่ได้ใช้ SMS)
